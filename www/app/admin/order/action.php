...
class adminOrder extends Curswork\ActionClass
{
    /**
     * Контроллер головної сторінки замовлень
     * 
    ...
    public function action_edit($params, $data)
    {
        ...
        if (count($data['task_list']) == 0) {
            header("HTTP/1.1 400 Bad Request");
            $this->response->redirect('/admin/order/delete/' . $data['order_id']);
        }
